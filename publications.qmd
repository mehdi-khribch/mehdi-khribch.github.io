...
We introduce the $\tilde{\rho}$-posterior, a generalized $\rho$-posterior and a modified version of the $\rho$-posterior of Baraud (2020), obtained by replacing the supremum over competitor parameters with a softmax aggregation. Unlike the original $\rho$-posteriors, whose minimax-theoretic analysis relies on testing arguments that preclude tractable computation, we show that $\tilde{\rho}$-posteriors admit a PAC-Bayesian analysis. This yields finite-sample oracle inequalities with explicit convergence rates that inherit the key robustness properties of the original framework, in particular graceful degradation under model misspecification and data contamination. Crucially, the PAC-Bayesian oracle inequalities extend to variational approximations of the $\tilde{\rho}$-posterior, providing theoretical guarantees for tractable inference. Numerical experiments on exponential families, regression, and real-world datasets confirm that the resulting variational procedures achieve robustness competitive with theoretical predictions at computational cost comparable to standard variational Bayes.
:::
